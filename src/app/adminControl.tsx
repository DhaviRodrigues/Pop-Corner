...
export default function AdminControl() {
  ...
  const loadAdmins = async () => {
    setLoading(true);
    try {
      const adminList = await UserAdmin.fetchAllAdmins();
      setAdmins(adminList);
    } catch (error: any) {
...
const handleAddAdmin = async () => {
    try {
      await UserAdmin.addAdmin(emailInput);
  ...
  const handleRemoveAdmin = async (targetUserId: string, targetUserEmail: string) => {
    try {
      await UserAdmin.removeAdmin(targetUserId, targetUserEmail, currentUserEmail);
