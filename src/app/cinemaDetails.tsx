...
// Componente para Estrelas Dinâmicas (Leitura)
...
export default function CinemaDetailsScreen() {
  ...
  // AQUI OCORRE A BLINDAGEM: Verifica se é Admin no Contexto OU no Firebase
  const isContextAdmin = user 
    ? ((typeof (user as any).getIsAdmin === 'function' && (user as any).getIsAdmin() === true) || ((user as any).isAdmin === true) || ((user as any).isAdm === true))
    : false;
  ...
  const handleDeleteCinema = async () => {
    ...
    try {
        ...
        alert("Falha ao deletar cinema. Verifique sua permissão.");
    }
  };
  ...
  const navigateToMap = () => {
    router.push({ pathname: '/map', params: { focusId: id } });
  };

  if (loading) {
    return <View style={[cinemaDetailsStyle.mainContainer, { justifyContent: 'center' }]}><ActivityIndicator size="large" color="#FFFEB2" /></View>;
  }

  if (!cinema) {
    return <View style={cinemaDetailsStyle.mainContainer}><Text style={textStyle.detailsTitleText}>Cinema não encontrado</Text></View>;
  }
  ...
  const mockFilmesCartaz = cinema.filmesEmCartaz?.map((fId: string, index: number) => ({
      id: fId,
      nome: `Filme ${index + 1}`,
      image: "https://placehold.co/200x300/1A1A1A/FFFEB2?text=Poster"
  })) || [];

  return (
    <View style={cinemaDetailsStyle.mainContainer}> 
      
      {/* 1. HEADER SEGURO (3 Colunas) */}
      <View style={[movieStyle.filmesHeader, { 
          ...
          <View style={{ width: 90, flexDirection: 'row', justifyContent: 'flex-end', gap: 10 }}>
              {isAdmin && (
                  <>
                    <TouchableOpacity 
                        onPress={() => router.push({ pathname: '/addTheater', params: { editId: id } })}
                        style={{ 
      ...
      >
        <View style={{ width: '100%', height: 250 }}>
            <Image 
                source={{ uri: cinema.url_imagem || cinema.imagem || "https://placehold.co/600x400/D9D9D9/A62103?text=Sem+Imagem" }} 
        ...
        <View style={cinemaDetailsStyle.contentWrapper}>
            ...
            <View style={{ marginBottom: 20 }}>
                <DynamicStarsDisplay rating={cinema.avaliacao || 0} />
            </View>

            <Text style={cinemaDetailsStyle.sectionTitle}>Filmes em Cartaz</Text>
            <FlatList
                horizontal
                data={mockFilmesCartaz}
            ...
            <View style={cinemaDetailsStyle.btnSessoesWrapper}>
                <ButtonY title="Ver Sessões" onPress={() => console.log('Abre Sessões')} />
            </View>

            <View style={[movieStyle.detailsSectionGrey, { marginTop: 30, padding: 15, borderRadius: 25 }]}>
                ...
                <View style={{ width: '100%', alignItems: 'center' }}>
                    <TextInput 
                        ...
                        value={myReview}
                        onChangeText={(text) => { if (text.length <= 300) setMyReview(text); }}
                        multiline={true}
                        textAlignVertical="top"
                        style={{
                            width: '100%', maxWidth: 350, height: 100, backgroundColor: '#FFF', 
                            borderRadius: 8, padding: 12, color: '#000', fontFamily: 'Poppins-Regular',
                        }}
                    />
                ...
                <View style={movieStyle.detailsButtonWrapper}>
                    {isSubmittingReview ? (
                        <ActivityIndicator size="small" color="#FFFEB2" />
                    ) : (
                ...
                {cinema.comentarios && cinema.comentarios.length > 0 ? cinema.comentarios.map((rev: any, index: number) => (
                    <View key={index} style={movieStyle.detailsReviewItem}>
                        <View style={movieStyle.detailsReviewAvatar}>
                            {rev.profilePic ? (
                                <Image source={{ uri: rev.profilePic }} style={{ width: '100%', height: '100%', borderRadius: 25 }} />
                            ) : (
                        ...
                        <View style={movieStyle.detailsReviewContent}>
                            <Text style={textStyle.detailsReviewUser}>
                              {rev.user || "Usuário"} <Text style={textStyle.detailsReviewStars}>
      ...
      <Modal visible={showDeleteModal} transparent={true} animationType="fade">
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.7)' }}>
            <View style={{ width: '90%', maxWidth: 320, backgroundColor: COLORS.primary, padding: 20, borderRadius: 15, alignItems: 'center' }}>
                ...
                <TextInput 
                    ...
                    style={{ width: '100%', backgroundColor: '#FFF', padding: 10, fontFamily: 'Poppins-Bold', borderRadius: 5, marginBottom: 20 }} 
                />
                <View style={{ flexDirection: 'row', gap: 10 }}>
                    <TouchableOpacity onPress={() => setShowDeleteModal(false)} style={{ padding: 10, backgroundColor: '#000000', borderRadius: 5 }}>
                        <Text style={{color: '#FFF', fontFamily: 'Poppins-Bold'}}>Cancelar</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={handleDeleteCinema} style={{ padding: 10, backgroundColor: COLORS.gold, borderRadius: 5 }}>
                        <Text style={{color: '#000000', fontFamily: 'Poppins-Bold'}}>Confirmar</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </View>
