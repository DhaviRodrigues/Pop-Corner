...
export default function CinemaDetailsScreen() {
  ...
  // Estados de Admin (Baseado no seu código de referência)
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [adminPassword, setAdminPassword] = useState("");
  const [isAdminState, setIsAdminState] = useState(false);

  // Verificação robusta que combina o Contexto com a variável de Estado
  const isContextAdmin = user
    ? (user as any).isAdmin === true ||
      (user as any).isAdm === true ||
      (typeof (user as any).getIsAdmin === "function" &&
        (user as any).getIsAdmin() === true)
    : false;
  ...
  useEffect(() => {
    ...
    // Função exata de checagem do código de referência
        ...
        try {
          ...
          for (const filmeId of cinema.filmesEmCartaz) {
            ...
            if (filmeSnap.exists()) {
              ...
              // Garante que imagens de Storage (Supabase/Firebase) carreguem perfeitamente
              let pathImg =
                dadosFilme.image ||
                dadosFilme.url_imagem ||
                dadosFilme.imagem ||
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      const userName =
        typeof (user as any)?.getName === "function"
          ? (user as any).getName()
          : (user as any)?.name || "Usuário";
      const userPic =
        typeof (user as any)?.getProfilePicture === "function"
  ...
  if (loading)
    return (
      <View
        style={[cinemaDetailsStyle.mainContainer, { justifyContent: "center" }]}
      >
  ...
  if (!cinema)
    return (
      <View style={cinemaDetailsStyle.mainContainer}>
        <Text style={textStyle.detailsTitleText}>Cinema não encontrado</Text>
      </View>
    );
      ...
      >
        <View style={{ width: "100%", height: 250 }}>
          {cinema.url_imagem || cinema.imagem ? (
            <Image
              source={{ uri: cinema.url_imagem || cinema.imagem }}
        ...
        <View style={cinemaDetailsStyle.contentWrapper}>
          <View style={cinemaDetailsStyle.titleRow}>
            <Text style={cinemaDetailsStyle.cinemaName} numberOfLines={2}>
              {cinema.nome}
            </Text>
            <TouchableOpacity
              style={cinemaDetailsStyle.mapButton}
            ...
            >
              <Image
                ...
                resizeMode="contain"
              />
              <Text style={cinemaDetailsStyle.mapDistanceText}>
                {distanciaText}
              </Text>
            </TouchableOpacity>
          </View>
              ...
              <View style={cinemaDetailsStyle.moviePosterWrapper}>
                {item.image ? (
                  <Image
                    ...
                    style={cinemaDetailsStyle.moviePoster}
                  />
                ) : (
                  <View
                    style={[
                      cinemaDetailsStyle.moviePoster,
                      {
                        backgroundColor: "#333",
                        justifyContent: "center",
                        alignItems: "center",
                ...
                <Text style={cinemaDetailsStyle.movieName} numberOfLines={2}>
                  {item.nome}
                </Text>
              </View>
            )}
            ListEmptyComponent={
              <Text style={textStyle.detailsInfoLabel}>
          ...
          <View style={cinemaDetailsStyle.btnSessoesWrapper}>
            <ButtonY
              title="Ver Sessões"
              onPress={() => console.log("Abre Sessões")}
            />
          </View>

          <View
            style={[
              movieStyle.detailsSectionGrey,
              { marginTop: 30, padding: 15, borderRadius: 25 },
            ]}
          >
            <Text
              style={[
                textStyle.detailsSectionTitle,
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Text
                    style={{
                      fontSize: 36,
                      color: star <= userRating ? "#FFFEB2" : "#666",
                      fontFamily: "Poppins-Regular",
                    }}
                  >
                    ★
            ...
            <View style={{ width: "100%", alignItems: "center" }}>
              <TextInput
                ...
                value={myReview}
                onChangeText={(text) => {
                  if (text.length <= 300) setMyReview(text);
                }}
                multiline={true}
          ...
          <View style={movieStyle.detailsSectionGrey}>
            <Text style={textStyle.detailsSectionTitle}>
              Avaliações e Comentários
            </Text>
            {cinema.comentarios && cinema.comentarios.length > 0 ? (
              cinema.comentarios.map((rev: any, index: number) => (
                <View key={index} style={movieStyle.detailsReviewItem}>
                  <View style={movieStyle.detailsReviewAvatar}>
                    {rev.profilePic ? (
                      <Image
                        source={{ uri: rev.profilePic }}
                        style={{
                          width: "100%",
                          height: "100%",
                          borderRadius: 25,
                  ...
                  <View style={movieStyle.detailsReviewContent}>
                    ...
                    <Text
                      style={textStyle.detailsReviewText}
                      numberOfLines={10}
                    >
                      {rev.comment}
                    </Text>
