...
export default function CinemaDetailsScreen() {
      ...
      if (auth.currentUser) {
        try {
          ...
          if (snap.exists()) {
            ...
            if (data.isAdm === true || data.isAdmin === true) setIsAdmin(true);
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (cinema?.comentarios && currentUserUid) {
      const existingReview = cinema.comentarios.find((c: any) => c.uid === currentUserUid);
      if (existingReview) {
        setHasReviewed(true);
  ...
  }, [cinema?.comentarios, currentUserUid]);
        ...
        try {
          ...
          for (const filmeId of cinema.filmesEmCartaz) {
            ...
            if (filmeSnap.exists()) {
              ...
              if (pathImg && pathImg.startsWith('/')) pathImg = `https://image.tmdb.org/t/p/w500${pathImg}`;
              const movieTitle = dadosFilme.nome || dadosFilme.titulo || dadosFilme.title || dadosFilme.original_title || "Filme sem título";

              listaFilmesBuscados.push({ id: filmeId, nome: movieTitle, image: pathImg });
            }
          }
  ...
  const handleDeleteCinema = async () => {
    if (!adminPassword) return Alert.alert("Aviso", "Digite a senha para confirmar.");
    if (isAdmin) {
      try {
  ...
  const handleSubmitReview = async () => {
    ...
    if (!id) return alert("Erro ao identificar o cinema.");

    try {
        setIsSubmittingReview(true);

        const userName = typeof (user as any)?.getName === 'function' ? (user as any).getReviewName ?? (user as any).getName() : ((user as any)?.name || "Usuário");
        ...
        const { updatedComments, newAverage, pipokaMessage } = await submitReviewService({
          ...
          userRating,
          myReview,
          currentComments: cinema?.comentarios || [],
          hasReviewed,
          pipokaAmount: 300,
          itemTitle: cinema?.nome || "Cinema"
        });
        ...
        <View style={cinemaDetailsStyle.contentWrapper}>
            ...
            <View style={[movieStyle.detailsSectionGrey, { marginTop: 30, padding: 15, borderRadius: 10 }]}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
                  <Text style={[textStyle.detailsSectionTitle, { textAlign: 'center', marginBottom: 0 }]}>
                    {hasReviewed ? "Sua Avaliação Atual" : "Avaliar Cinema"}
                  </Text>
                  {hasReviewed && (
